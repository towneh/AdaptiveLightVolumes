...
- Hardware ray-tracing bake path using inline RT in a compute shader against a `RayTracingAccelerationStructure` (Unity 6.4+, D3D12, DXR-capable GPU). Auto-fallback to CPU on unsupported hardware with the failure reason logged.
